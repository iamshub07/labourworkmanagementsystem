...
async (req,res) => { 
  ...
  const {name, userid, email, password, isowner,designation } = req.body;
    ...
    if (isowner) userFields.isowner = isowner;
...
router.post('/getcurrentuser', auth, async (req, res) => {
    try {
        const userdetail = await UserDetail.findOne({ user: req.user.id  }).populate('user', ['name','isowner','designation']);
        if (!userdetail) {
